...
class MyClient(discord.Client):
    ...
    async def on_message(self,message):
        ...
        urlCheems = 'https://imgur.com/a/phKdjDz'
        urlBalls = 'https://pbs.twimg.com/media/EgsZwobU0AAa-qe.jpg'
       ...
       #length check
        if len(messageContent) > 0:
            ...
            for word in liCringe:
                if word in messageContent:
                    await message.reply("Crimge" + urlCheems)
            ...
            for word in liFat:
                if word in messageContent:
                    await message.reply(user_jam + "Your ride is here. " + urlForklift)

            #WELD

            for word in liWeld:
                if word in messageContent:
                    await message.reply(urlArk)

            #WEED

            for word in liWeed:
                if word in messageContent:
                    await message.reply("DUDE WEED")
